	...
	if ( ! class_exists( 'CBXWPSimpleaccounting_Settings_API' ) ):
		...
		class CBXWPSimpleaccounting_Settings_API {
			...
			function callback_multiselect( $args, $value = null ) {
				...
				if ( $args['placeholder'] == '' ) {
					$args['placeholder'] = esc_html__( 'Please Select', 'cbxwpsimpleaccounting' );
				}
			...
			function callback_file( $args, $value = null ) {
				...
				$label = isset( $args['options']['button_label'] ) ?
					...
					esc_html__( 'Choose File', 'cbxwpsimpleaccounting' );
			...
			function callback_repeat( $args ) {
				...
				if($allow_new){
					$html .= '<p style="text-align: center;"><a data-index="'.intval($index).'" data-busy="0" data-field_name="'.$args['name'].'" data-section_name="'.$section_name.'" data-option_name="'.$option_name.'" class="button button-secondary form-table-fields-new" href="#">'.esc_html__('Add New', 'cbxwpsimpleaccounting').'</a></p>';
				}
