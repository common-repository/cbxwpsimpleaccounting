...
class CBXWPSimpleaccounting_Upload
{
    ...
    protected function initialize() {
        ...
        switch ($this->get_server_var('REQUEST_METHOD')) {
            ...
            case 'HEAD':
                ...
                break;
            case 'GET':
                $this->get($this->options['print_response']); //for list, download
                break;
            case 'PATCH':
            case 'PUT':
            case 'POST':
    ...
    protected function set_additional_file_properties($file) {
	    ...
	    //$file->deleteUrl .= '&action=cbxwpsimpleaccounting_delete_attachments';
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
	    ...
	    $allowed_attachments = $settings->get_option( 'allowed_attachment', 'cbxwpsimpleaccounting_attachments', array() );
	    $allowed_mime_types = apply_filters( 'cbxwpsimpleaccounting_mime_types', get_allowed_mime_types() );

	    $allowed_mime_types_final = array();

	    foreach ($allowed_attachments as $allowed_attachment){
	    	if(isset($allowed_mime_types[$allowed_attachment])){
			    $allowed_mime_types_final[] = $allowed_mime_types[$allowed_attachment];
		    }
	    }
	    ...
	    if(extension_loaded( 'fileinfo' )){
		    ...
		    $real_mime = finfo_file( $finfo, $uploaded_file );
		    finfo_close( $finfo );

		    if(!in_array($real_mime, $allowed_mime_types_final)){
...
}//end class CBXWPSimpleaccounting_Upload
